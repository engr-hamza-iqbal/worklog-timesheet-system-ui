...
export default function ReportsPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-[1680px] px-4 py-8 sm:px-6 lg:px-10 2xl:px-14">
      ...
      <form onSubmit={(event) => { event.preventDefault(); load(); }} className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><label className="text-xs font-medium text-slate-600">Start date<input type="date" max={filters.endDate || undefined} value={filters.startDate} onChange={(e) => setFilters({ ...filters, startDate: e.target.value })} className="mt-1 rounded-md border px-3 py-2 text-sm" /></label><label className="text-xs font-medium text-slate-600">End date<input type="date" min={filters.startDate || undefined} value={filters.endDate} onChange={(e) => setFilters({ ...filters, endDate: e.target.value })} className="mt-1 rounded-md border px-3 py-2 text-sm" /></label><button disabled={loading} className="inline-flex items-center gap-2 rounded-md bg-slate-900 px-3 py-2 text-sm text-white disabled:opacity-50"><BarChart3 size={15} className={loading ? 'animate-pulse' : ''} />{loading ? 'Updating report' : 'Run report'}</button></form>
      {error && <div className="mb-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
      {loading && !report ? <div className="flex min-h-64 items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white text-sm text-slate-500 shadow-sm"><RefreshCw className="animate-spin" size={22} />Loading reports...</div> : report && <div className="relative"><div className="grid gap-5 xl:grid-cols-2"><MetricTable title="Hours by project" rows={report.byProject} columns={[{ key: 'project', label: 'Project', value: (row) => `${row.projectName} · ${row.clientName}` }, { key: 'hours', label: 'Hours', value: (row) => row.hours }, { key: 'value', label: 'Billable value', value: (row) => `$${row.billableValue}` }]} /><MetricTable title="Billable value by client" rows={report.byClient} columns={[{ key: 'client', label: 'Client', value: (row) => row.clientName }, { key: 'hours', label: 'Hours', value: (row) => row.hours }, { key: 'value', label: 'Billable value', value: (row) => `$${row.billableValue}` }]} /><MetricTable title="Hours by employee" rows={report.byEmployee} columns={[{ key: 'employee', label: 'Employee', value: (row) => row.userName }, { key: 'email', label: 'Email', value: (row) => row.email }, { key: 'hours', label: 'Hours', value: (row) => row.hours }]} /><MetricTable title="Entry status breakdown" rows={report.byStatus} columns={[{ key: 'status', label: 'Status', value: (row) => row.status }, { key: 'entries', label: 'Entries', value: (row) => row.entries }, { key: 'hours', label: 'Hours', value: (row) => row.hours }]} /></div>{loading && <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-white/60 backdrop-blur-[1px]"><div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 shadow-md"><RefreshCw className="animate-spin" size={18} />Refreshing reports</div></div>}</div>}
    </main>
  );
}
